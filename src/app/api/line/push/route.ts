...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.userId || !body.articleUrl || !body.summaryTitle || !body.summaryText || !body.templateId) {
      return NextResponse.json({ error: '必須パラメータが不足しています (userId, articleUrl, summaryTitle, summaryText, templateId)' }, { status: 400 });
    }
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.error || '個別配信に失敗しました' }, { status: 500 });
    }
    return NextResponse.json({ success: true, sentAt: result.sentAt });
  } catch (error) {
    console.error('[push]', error);
    return NextResponse.json({ error: '個別配信処理中にエラーが発生しました' }, { status: 500 });
  }
}
