...
import { testBroadcastArticle } from '@/lib/line/messaging';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await testBroadcastArticle(req);
    if (!result.success) {
      return NextResponse.json({ error: result.error || 'テスト配信に失敗しました' }, { status: 500 });
    }
    return NextResponse.json({ success: true, sentAt: result.sentAt });
  } catch (error) {
    console.error('[test-broadcast]', error);
    return NextResponse.json({ error: 'テスト配信処理中にエラーが発生しました' }, { status: 500 });
  }
}
